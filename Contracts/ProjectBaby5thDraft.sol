...
contract Mkopo {
    ...
    function mint(string memory NFTurl, uint256 price) public payable {
        ...
        require(msg.value >= mintingPrice, 'Insufficient amount for minting');
        require(mintedWallet[msg.sender] < 6, 'Up to five minting is allowed, you exceeded the limit');
    ...
    */
    ...
    function failedToPayBack(AfterBorrowingMoney memory abm) public {
        require(msg.sender == developerAddress, 'You should be developer');
        ...
        for (uint256 i=0; i < assetsList.length; i++) {
            ...
            if(keccak256(abi.encodePacked(nftData[assetsList[i]][abm.deniDetail.borrower])) != keccak256(abi.encodePacked(""))) { // sawa hii inareturn string, na error yetu hapa ni kuwa inashindwa ku-convert hii returned data into bool to be executed in if...
        ...
        for (uint256 i=0; i < ListOfAfterBorrowingMoney.length; i++) {  
            if (ListOfAfterBorrowingMoney[i].issuedAt == abm.issuedAt) {  // How to compare two struct.... Hapa hatuwezi tuka-compare struct nzima ni lazima tuchague unique data kwenye hizi struct then tu-compare... sasa hapa nahisi pasco inabidi uongeze "struct id" field in your struct for everything to go well
                index = i;    // Ok nishapata jibu kwenye hii struct yetu data muhimu ambayo NI LAZIMA IWE UNIQUE NI issuedAt.. Kumbuka time coz inahusisha had miliseconds and microseconds kuwa sawa nahisi itakua ngumu so this is our uinque identifier...
            }
        }
    ...
    function payback(AfterBorrowingMoney memory abm) public payable{
        require(msg.sender == abm.deniDetail.borrower, 'Your not a borrower of this debt');
        require(msg.value >= abm.returnedAmount, 'Incorrect balance passed!');
        payable(abm.lender).transfer(msg.value);
